...
Display 1 H.264/H.265 video simultaneously on HDMI monitor.
...
- Variable `input_video_file (const gchar)` to represent H.264/H.265 video input file.
...
```
This application accepts a command-line argument which points to a H.264/H.265 file.
...
if ((strcasecmp ("h264", ext) == 0) || (strcasecmp ("264", ext) == 0)) {
  ...
  user_data.decoder = gst_element_factory_make ("omxh264dec",
                          "h264-decoder");
} else if ((strcasecmp ("h265", ext) == 0) || (strcasecmp ("265", ext) == 0)) {
...
```
>RZ/V2H platform supports playing 2 1920x1080, 30 fps videos
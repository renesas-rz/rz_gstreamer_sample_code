...
main (int argc, char *argv[])
{
  ...
  /* The display position of the first display is (0,0), of the second display is (availbe_width_screen[0],0) */
  /* Create first pipeline */
  /* 
   * (availbe_width_screen[0]xavailbe_height_screen[0]) is value of first screen and 
   * (availbe_width_screen[1]xavailbe_height_screen[1]) is value of second screen 
   */
  display.x = POSITION_X;
  display.y = POSITION_Y;
  display.width = availbe_width_screen[0];
  display.height = availbe_height_screen[0];
  video_bus_watch_id_1 =
      create_video_pipeline (&video_pipeline_1, input_video_file_1, &display,
      &shared_data);

  /* Create second pipeline */
  if( count_screens > 1 ) {
    display.x =  availbe_width_screen[0];
    display.y = POSITION_Y;
    display.width = availbe_width_screen[1];
    display.height = availbe_height_screen[1];
  }else {
  ...
  video_bus_watch_id_2 =
      create_video_pipeline (&video_pipeline_2, input_video_file_2, &display,
      &shared_data);

...
#define ARG_HEIGHT         4
#define ARG_COUNT          5
#define NUM_MIPI_RESOLUTIONS 4
#define NUM_USB_RESOLUTIONS  8
...
const char *RZV2N_RZV2H_mipi_resolutions[] = {
  "640x480",
  "1280x720",
  "1920x1080",
  NULL,
};
const char *RZG2L_RZV2L_RZG3E_mipi_resolutions[] = {
  "1280x960",
  "1920x1080",
  NULL,
};
...
const char *RZV2N_RZV2H_usb_resolutions[] = {
  "320x240",
  "640x480",
  "800x600",
  "960x720",
...
const char *RZG2L_RZV2L_RZG3E_usb_resolutions[] = {
  "320x240",
  "640x480",
  "800x600",
  "960x720",
...
const char *mipi_resolutions[NUM_MIPI_RESOLUTIONS];
const char *usb_resolutions[NUM_USB_RESOLUTIONS];
...
OMX_ERRORTYPE event_handler(OMX_HANDLETYPE hComponent, OMX_PTR pAppData, OMX_EVENTTYPE eEvent,
...
enum board_name get_board_name() {
  ...
  if (OMX_ComponentNameEnum(cComponentName, OMX_MAX_STRINGNAME_SIZE, 0) != OMX_ErrorNone) {
    goto cleanup;
  }

  if (OMX_GetHandle(&hComponent, cComponentName, NULL, &pCallBacks) != OMX_ErrorNone) {
    goto cleanup;
  }

  if (OMX_GetComponentVersion(hComponent, cComponentName, &pComponentVersion,
                              &pSpecVersion, &pComponentUUID) == OMX_ErrorNone) {
...
main (int argc, char *argv[])
{
  ...
  if(board == RZG2L_RZV2L || board == RZG3E) {
    if (user_data.camera == MIPI_CAMERA) {
      memcpy(mipi_resolutions, RZG2L_RZV2L_RZG3E_mipi_resolutions, sizeof(RZG2L_RZV2L_RZG3E_mipi_resolutions));
    } else {
      memcpy(usb_resolutions, RZG2L_RZV2L_RZG3E_usb_resolutions, sizeof(RZG2L_RZV2L_RZG3E_usb_resolutions));
    }
  } else if(board == RZV2N_RZV2H) {
    if (user_data.camera == MIPI_CAMERA) {
      memcpy(mipi_resolutions, RZV2N_RZV2H_mipi_resolutions, sizeof(RZV2N_RZV2H_mipi_resolutions));
    } else {
      memcpy(usb_resolutions, RZV2N_RZV2H_usb_resolutions, sizeof(RZV2N_RZV2H_usb_resolutions));
    }
  }

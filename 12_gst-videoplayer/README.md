...
A simple text-based video player that can play MP4 (H.264) files. Supported features: play, pause, stop, resume, and seek video files. You can input a single file or a whole directory.
...
-	 Variable `video_parser (GstElement)`: A GStreamer element to parse H.264 stream to AVC/HEVC format which omxh264dec can recognize and process.
